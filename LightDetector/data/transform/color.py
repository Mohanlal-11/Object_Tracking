...
def augment_hue(image, delta_hue):
    hue_val = random.uniform(-delta_hue, delta_hue)
    hsv_image = cv2.cvtColor(image.astype(np.float32), cv2.COLOR_BGR2HSV)

    hsv_image[:, :, 0] = (hsv_image[:, :, 0] * hue_val) % 180 # Hue values are in the range [0, 180) in openCV
...
def color_aug_and_norm(meta, kwargs):
    ...
    if 'hue' in kwargs and random.randint(0,1):
        img = augment_hue(image=img, delta_hue=kwargs['hue'])
