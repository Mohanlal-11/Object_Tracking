...
from .Add_noise import Noise_Polygon
import random
...
class Pipeline:
    ...
    def __init__(self, cfg: Dict, keep_ratio: bool):
        ...
        # self.shape_transform = ShapeTransform(keep_ratio)
        self.color = functools.partial(color_aug_and_norm, kwargs=cfg)

    def __call__(self, dataset: Dataset, meta: Dict, dst_shape: Tuple[int, int], annotation=None):
        if random.random() > 0.5:
            if annotation is not None:
                meta = Noise_Polygon(meta=meta)
