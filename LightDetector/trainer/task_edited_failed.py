...
class TrainingTask(LightningModule):
    ...
    def __init__(self, cfg, evaluator=None, ckpt=None, mosaic=False):
        ...
        if ckpt:
            ...
            # logger.info("Freezing early/initial convolutional layers/stages of backbone i.e. ShufflenetV2 to retain learned low-level features.")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return {"val_loss": loss, "predictions": dets}
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        # Pass the average validation loss to the scheduler
        self.trainer.lr_scheduler[0].step(avg_val_loss)
        ...
        # Perform evaluation using the predictions
        results = gather_results(all_predictions)
        if dist.is_available() and dist.is_initialized():
            all_results = gather_results(results)
        else:
            all_results = results

        # If results exist, evaluate
        if all_results:
            eval_results = self.evaluator.evaluate(
                all_results, self.cfg.save_dir, rank=self.local_rank
            )

            # Optionally log evaluation results
            if eval_results.get('F1-score'):
                data = [['F1-Score', float(eval_results['F1-score']*100)], 
                        ['Mean Precision@0.5:0.95', float(eval_results["Mean Precision"]*100)],
                        ['Recall (maxDets=100)', float(eval_results["Mean Recall"]*100)]]
    ...
    def configure_optimizers(self):
        ...
        if name == "ReduceLROnPlateau":
            # Ensure that you monitor the validation loss (or any metric you wish)
            scheduler = {
